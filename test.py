...
def draw(filename, result):
    ...
    for class_name, lx, ly, rx, ry, ang, prob in res:
        ...
        # plt.imshow(img)
        # plt.show()
    img.save(os.path.join('results', 'resnet34', os.path.split(filename)[-1]))
...
if __name__ == '__main__':
    model = ResNet(34).cuda()
    device = torch.device('cuda')

    model.load_state_dict(torch.load('saved_models/AirPlane/best_state_dict.pth'))
    model.eval()
